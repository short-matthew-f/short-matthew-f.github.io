  function buildSteps(){
    steps=[{title:'Your shape',text:'This is the straight-line cut pattern you drew.',foldStage:0,creases:0,cut:0,reveal:0,group:-1},{title:'Crease construction',text:'The theorem solver builds the straight-skeleton crease network and divides the paper into rigid faces.',foldStage:0,creases:1,cut:0,reveal:0,group:-1}];
    solution.groups.forEach((group,g)=>{
      const packets=group.length,faces=new Set(group.flatMap(op=>op.moving)).size;
      steps.push({title:packets>1?`Compound fold ${g+1}`:`Packet fold ${g+1}`,text:packets>1?`${packets} solver corridors collapse together. Each hinge carries its entire already-folded packet instead of moving a single face.`:`Fold the whole ${faces}-face packet around the solver-selected crease.`,foldStage:g+1,creases:1,cut:0,reveal:0,group:g});
    });
    const q=solution.sequenceError<1.5?'The packet sequence reaches the computed flat state.':'The solver packets are now flat; the common cut line is shown from their resulting stack.';
    steps.push({title:'One flat stack',text:q,foldStage:solution.groups.length,creases:1,cut:0,reveal:0,group:-1});
    steps.push({title:'Make one straight cut',text:'The blade passes once through the folded stack along the line shared by the target edges.',foldStage:solution.groups.length,creases:1,cut:1,reveal:0,group:-1});
    steps.push({title:'Unfold the result',text:'Reverse the same packet folds. The one cut opens back out into the shape you drew.',foldStage:0,creases:0,cut:0,reveal:1,group:-1});
  }
  ...
  function faceWorld(face,T){return face.verts.map(id=>TA(T,[solution.vertices[id].x,solution.vertices[id].y,0]));}
  ...
  function draw3D(){
    const Ts=computeTransformsFor(solution,visual.foldStage);const faceDraw=solution.faces.map((f,i)=>{const world=faceWorld(f,Ts[i]),avg=world.reduce((s,p)=>s+project(p).depth,0)/world.length,raise=world.reduce((s,p)=>s+Math.abs(p[2]),0)/world.length;return{f,i,world,avg,raise};}).sort((a,b)=>a.avg-b.avg);
    ...
    faceDraw.forEach(o=>{const pp=pathProjected(o.world);const nz=faceNormal(o.world)[2];ctx.save();ctx.fillStyle=nz>=0?'#fffdf7':'#f1e6d2';ctx.globalAlpha=.98;ctx.fill();ctx.strokeStyle='rgba(83,72,56,.22)';ctx.lineWidth=1;ctx.stroke();ctx.restore();});
    ...
    if(visual.creases>.02){
      const s=steps[stepIndex]||{},active=s.group>=0?new Set(solution.groups[s.group].map(op=>op.edgeIndex)):new Set();
      solution.edges.forEach((e,ei)=>{if(edgeKind(e)!=='crease')return;const [a,b]=edgeWorld(e,Ts),pa=project(a),pb=project(b),on=active.has(ei);ctx.save();ctx.globalAlpha=Math.max(.15,visual.creases*(on?1:.48));ctx.strokeStyle=e.assignment==='m'?'#bd4a3d':e.assignment==='v'?'#3d718e':'#948c80';ctx.lineWidth=on?3.2:1.15;ctx.setLineDash(on?[]:[5,5]);ctx.beginPath();ctx.moveTo(pa.x,pa.y);ctx.lineTo(pb.x,pb.y);ctx.stroke();ctx.restore();});
    }
