...
function repair(src){
  ...
  injected="  src=src.replace("+JSON.stringify(pegLoop)+",'');\n"+
    "  src=mustReplace(src,\"mergeSave(save,parsed);}catch(e){}\",\"mergeSave(save,parsed);if(parsed&&parsed.pegMeta)save.pegMeta=clone(parsed.pegMeta);}catch(e){}\",'pegMeta dynamic-key restore');\n";
  ...
  src=src.replace("var VERSION='1.11.7';","var VERSION='1.12.2';");
  src=src.replace(/engine-v117-bootstrap\.js/g,'engine-v122-bootstrap.js');
  return src;
}
fetch('engine-v117-loader.js?v=20260818-117')
 ...
 .then(function(src){
   ...
   try{fn=new Function(repaired+'\n//# sourceURL=engine-v122-loader-runtime.js');}
