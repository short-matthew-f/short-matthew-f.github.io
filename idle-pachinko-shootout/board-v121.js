(function(){
'use strict';
var board=document.getElementById('board'),wrap=board&&board.closest('.board-wrap'),api=null,layer=null,lastSig='',nodes=[];
...
function nodeFor(index){var n=nodes[index];if(n)return n;n=document.createElement('span');n.className='peg-hardware';n.setAttribute('data-hardware-index',String(index));n.innerHTML='<i class="peg-socket"></i><i class="peg-face"><b class="peg-symbol"></b></i><i class="peg-pips"></i>';nodes[index]=n;layer.appendChild(n);return n;}
...
function render(force){if(!api)return;ensureLayer();if(!layer)return;var s=snapshot(),sig=signature(s),i,p,t,lvl,n,sym,ps;if(!force&&sig===lastSig)return;lastSig=sig;for(i=0;i<POS.length;i++){p=POS[i];t=typeAt(i,s);lvl=(t==='n'||t==='pin')?0:levelAt(t,i,s);n=nodeFor(i);n.className='peg-hardware peg-'+t+(t!=='n'?' peg-special':'')+(lvl?' peg-upgraded':'');n.style.left=(p.x/393*100)+'%';n.style.top=(p.y/472*100)+'%';n.setAttribute('data-peg-type',t);n.setAttribute('data-peg-level',String(lvl));n.setAttribute('title',LABELS[t]||'Peg');sym=n.querySelector('.peg-symbol');if(sym&&sym.textContent!==(ICONS[t]||''))sym.textContent=ICONS[t]||'';ps=n.querySelector('.peg-pips');if(ps){var ph=pips(lvl);if(ps.innerHTML!==ph)ps.innerHTML=ph;}}}
function pulse(index){var n=nodes[index],face,anim;if(!n)return;face=n.querySelector('.peg-face');if(!face)return;if(face.animate){if(face.__pegAnim)try{face.__pegAnim.cancel();}catch(e){}anim=face.animate([{transform:n.classList.contains('peg-pin')?'translate(-50%,-53%) rotate(45deg) scale(1)':'translate(-50%,-53%) scale(1)',opacity:1},{transform:n.classList.contains('peg-pin')?'translate(-50%,-53%) rotate(45deg) scale(1.18)':'translate(-50%,-53%) scale(1.18)',opacity:.82},{transform:n.classList.contains('peg-pin')?'translate(-50%,-53%) rotate(45deg) scale(1)':'translate(-50%,-53%) scale(1)',opacity:1}],{duration:150,easing:'ease-out'});face.__pegAnim=anim;return;}n.classList.add('peg-hit');setTimeout(function(){n.classList.remove('peg-hit');},160);}
function scrubOldPinUi(){var label=wrap&&wrap.querySelector('.firing-pin-label');if(label&&label.parentNode)label.parentNode.removeChild(label);}
function rerender(){setTimeout(function(){render(true);},0);}
function boot(){api=window.__ipsAPI;if(!api||!api.snapshot||!board||!wrap)return false;ensureLayer();render(true);scrubOldPinUi();['ips:pegUpgrade','ips:pegMove','ips:pegSell','ips:upgrade'].forEach(function(name){document.addEventListener(name,rerender);});document.addEventListener('ips:peg',function(e){var d=e&&e.detail||{};if(d.index!==undefined&&d.index!==null)pulse(Number(d.index));else if(d.type==='pin')pulse(PIN);});window.addEventListener('resize',function(){render(true);});setInterval(function(){render(false);},4000);return true;}
