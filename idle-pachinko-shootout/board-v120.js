...
function scrubOldPinUi(){var label=wrap&&wrap.querySelector('.firing-pin-label'),ctx=wrap&&wrap.querySelector('.firing-pin-context'),small,tag,effect;if(label&&label.parentNode)label.parentNode.removeChild(label);if(ctx){small=ctx.querySelector('.peg-context-head small');tag=ctx.querySelector('.peg-context-head span');effect=ctx.querySelector('.peg-effect');if(small)small.textContent='BOARD HARDWARE';if(tag)tag.textContent='ANCHOR';if(effect)effect.textContent='Every ball passes through this launch point. It is permanent board hardware and has no upgrade, move, or sell controls.';}}
function boot(){api=window.__ipsAPI;if(!api||!api.snapshot||!board||!wrap)return false;ensureLayer();render(true);scrubOldPinUi();['ips:pegUpgrade','ips:pegMove','ips:pegSell','ips:upgrade','ips:runStart','ips:waveStart'].forEach(function(name){document.addEventListener(name,function(){setTimeout(function(){render(true);},0);});});document.addEventListener('ips:peg',function(e){var d=e&&e.detail||{};if(d.index!==undefined&&d.index!==null)pulse(Number(d.index));else if(d.type==='pin')pulse(PIN);});new MutationObserver(scrubOldPinUi).observe(wrap,{childList:true,subtree:true});window.addEventListener('resize',function(){render(true);scrubOldPinUi();});setInterval(function(){render(false);},650);return true;}
