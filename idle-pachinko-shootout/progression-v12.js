...
function apply(){if(!window.__ipsAPI)return;var s=snap(),m=normalize(read(),s),k,def,els,i,b,ok,afford;for(k in BLUEPRINTS){if(m.bought[k])continue;def=BLUEPRINTS[k];ok=eligible(def,s);afford=Number(s.coins||0)>=def.cost;els=document.querySelectorAll('[data-board="'+k+'"],[data-ammo-unlock="'+k+'"]');for(i=0;i<els.length;i++){b=els[i];b.classList.add('blueprint-gate');b.setAttribute('data-blueprint-family',k);if(!ok){b.disabled=true;b.textContent='CLEAR WAVE '+def.clear+' FOR BLUEPRINT';}else{b.disabled=!afford;b.textContent=(afford?'BUY ':'NEED ● ')+def.label+' · ● '+def.cost;}}}}
function buy(k){var def=BLUEPRINTS[k],s=snap(),m=normalize(read(),s);if(!def||m.bought[k]||!eligible(def,s)||Number(s.coins||0)<def.cost)return;window.__ipsAPI.grant(0,-def.cost,'BLUEPRINT PURCHASED');m.bought[k]=1;write(m);showToast('BLUEPRINT PURCHASED',def.desc+' can now be purchased.');var name=sheetName();if(name)window.__ipsAPI.openSheet(name);setTimeout(apply,30);}
