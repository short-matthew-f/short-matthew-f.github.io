...
var RESET_KEYS=[
  ...
  'ips-idle-v1','ips-telemetry-v1','ips-campaign-v1','ips-contracts-v1','ips-combat-telemetry-v1','ips-blueprints-v1','ips-loot-v1'
];
...
function removeGameData(){var i;for(i=0;i<RESET_KEYS.length;i++){try{localStorage.removeItem(RESET_KEYS[i]);}catch(e){}}
  ...
  try{
    ...
    for(i=0;i<localStorage.length;i++){
      ...
      if(/^ips-(v\d+|idle-|telemetry-|campaign-|contracts-|combat-telemetry-|blueprints-|loot-)/.test(k)||/^idle-pachinko-shootout/.test(k))remove.push(k);
    }
