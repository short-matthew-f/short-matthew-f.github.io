(function(){
'use strict';
if(window.__ipsFeel133)return;
window.__ipsFeel133=true;

var api=null,lastWallet=null,lastSlot=null,lastShot=null,lastImpactAt=0;
var AMMO_COLORS={standard:'#e1bd73',gold:'#ffd957',fire:'#ff9a3d',concuss:'#ffe39a',shrapnel:'#d8c5a5',poison:'#7bd56a',pierce:'#7de3ed',boom:'#ff7348',chain:'#c89aff',split:'#62d6c8',pin:'#b6bdc2'};
...
function onLaunch(d){var cap=q('.launcher-cap'),queue=$('bulletQueue'),color=colorForAmmo(d.ammo),layer=boardLayer();if(cap){cap.style.setProperty('--feel-color',color);pulse(cap,'feel-launch',190);}if(queue){queue.style.setProperty('--feel-color',color);pulse(queue,'feel-queue-kick',210);}if(layer&&cap)spawnAt(layer,cap,'feel-launch-ring','',color,430);}
function onPeg(d){var type=d.type||'normal',idx=d.index,node,layer=boardLayer(),color;if(type==='normal'||type==='pin')return;if(idx===undefined||idx===null)return;node=q('[data-hardware-index="'+idx+'"]');if(!node)return;color=colorForKind(type);node.style.setProperty('--feel-color',color);pulse(node,'feel-peg-hit',260);spawnAt(layer,node,'feel-peg-burst feel-'+safeClass(type),PEG_LABELS[type]||'',color,type==='boom'?780:580);}
function onSlot(d){var slots=qa('.slot-label'),el=slots[Number(d.slot)],layer=boardLayer(),color=colorForAmmo(d.ammo),high=Number(d.value)>=7,wrap=q('.board-wrap');lastSlot={value:Number(d.value||0),crit:!!d.crit,ammo:d.ammo};if(!el)return;el.style.setProperty('--feel-color',color);pulse(el,high?'feel-slot-jackpot':'feel-slot-hit',high?520:330);spawnAt(layer,el,'feel-slot-pop '+(high?'high ':'')+(d.crit?'crit':''),'×'+d.value,color,high?780:520);if(high&&wrap){wrap.style.setProperty('--feel-color',color);pulse(wrap,'feel-board-payoff',360);}}
...
function onEnemyHit(d){var t=performance.now(),kind=(lastShot&&lastShot.kind)||'normal',crit=!!d.crit,target,layer;if(t-lastImpactAt<48&&!crit)return;lastImpactAt=t;if(kind==='normal'&&!crit)return;target=q('.enemy.focused')||q('.enemy');layer=combatLayer();if(target)spawnAt(layer,target,'feel-impact feel-'+safeClass(kind)+(crit?' crit':''),'',(lastShot&&lastShot.color)||null,360);}
function onEnemyDeath(d){var combat=$('combat'),layer=combatLayer(),target=q('.enemy.focused')||q('.enemy'),boss=!!d.boss;if(combat)pulse(combat,boss?'feel-boss-down':'feel-kill',boss?700:260);if(boss&&target&&layer)spawnAt(layer,target,'feel-death-mark boss','BOSS DOWN','',1100);if(boss)feelToast('BOSS DOWN','Frontier loot incoming.','boss');}
...
function onUpgrade(d){var sheet=$('sheet'),layer,stamp;if(sheet){pulse(sheet,'feel-sheet-confirm',420);layer=makeLayer(sheet,'sheet-feel-layer');if(layer){stamp=document.createElement('i');stamp.className='feel-upgrade-stamp';stamp.textContent='UPGRADED';layer.appendChild(stamp);removeLater(stamp,620);}}feelToast('UPGRADE LOCKED IN',upgradeName(d),'upgrade');queueWalletDelta();}
function onPegUpgrade(d){feelToast('PEG UPGRADED',(PEG_LABELS[d.type]||'PEG')+' · LV '+d.level,'peg');queueWalletDelta();}
function onLoot(d){var card=q('#lootModal .loot-card'),newCard=$('newLootCard');if(card)pulse(card,'feel-loot-reveal',720);if(newCard)pulse(newCard,d.unique?'feel-unique-loot':'feel-new-loot',900);}
...
function onAmmoLoad(d){var queue=$('bulletQueue');if(queue){queue.style.setProperty('--feel-color',colorForAmmo(d.type));pulse(queue,'feel-ammo-loaded',420);}feelToast('ROUND LOADED',String(d.type||'special').toUpperCase(),'ammo');}
