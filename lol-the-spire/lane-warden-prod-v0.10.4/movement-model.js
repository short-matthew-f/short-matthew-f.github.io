...
if(!E||!L||!D)return;
const BUILD='P2-0.16.1',VERTICAL_EQUIV=.24,EPS=.0001;
...
function installTravel(b,start,targetLane,targetPos,template){targetPos=clamp(targetPos,.12,.86);const built=buildRoute(start,targetLane,targetPos,template?.crossing),route=built.points,length=Math.max(.001,routeLen(route)),fromLane=yLane(start.y),mult=durationMultiplier(b,fromLane,targetLane),speed=L.cfg.commanderSpeed/Math.max(.001,mult),duration=Math.max(.18,length/speed);b.commander.move=null;b.commander.travel={...(template||{}),from:fromLane,to:targetLane,fromPos:start.x,targetPos,route,routeLength:length,total:duration,remaining:duration,speed,movementMultiplier:mult,crossing:built.crossing};b.events.push({t:b.elapsed,type:'commander-route',from:fromLane,to:targetLane,targetPos,length,duration,multiplier:mult,crossing:built.crossing});return true}
