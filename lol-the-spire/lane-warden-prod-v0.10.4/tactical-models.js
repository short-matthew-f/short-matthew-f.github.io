(() => {
'use strict';
const BUILD='P2-0.21.0',MANIFEST='./assets/manifests/tactical-models.json?v=0.21.0';
const api={build:BUILD,ready:false,error:null,manifest:null,atlas:null,frames:null};
api.load=async()=>{if(api.promise)return api.promise;api.promise=(async()=>{const manifest=await fetch(MANIFEST),m=await manifest.json(),framesRes=await fetch(`${m.runtime.frames}?v=0.21.0`),frames=await framesRes.json(),img=new Image();img.decoding='async';img.src=`${m.runtime.atlas}?v=0.21.0`;await img.decode();api.manifest=m;api.frames=frames;api.atlas=img;api.ready=true;return api})().catch(e=>{api.error=String(e);return api});return api.promise};
