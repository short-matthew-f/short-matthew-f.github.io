...
// Compatibility surfaces stay alive for app.js, but presentation moves elsewhere.
...
tray.id='laneActionTray';tray.className='lane-action-tray';tray.hidden=true;
tray.innerHTML=`<div class="lane-tray-head"><div><b id="laneTrayTitle">LANE ACTIONS</b><small id="laneTrayStatus">Select a lane</small></div><button id="laneTrayClose" class="lane-tray-close" type="button" aria-label="Close lane actions">×</button></div><div class="lane-tray-actions"><button id="uiFortify" type="button">FORTIFY<small>28g</small></button><button id="uiOvercharge" type="button">OVERCHARGE<small>36g</small></button><button id="uiReinforce" type="button">REINFORCE<small>30g · 2 pulses</small></button></div><small id="laneTrayFeedback" class="lane-tray-feedback"></small>`;
battle.appendChild(tray);
...
function ensureLaneChips(){for(const card of laneButtons()){if(card.querySelector('.lane-move-chip'))continue;const chip=document.createElement('span');chip.className='lane-move-chip';chip.dataset.uiLane=card.dataset.lane;chip.textContent='WALK HERE';card.appendChild(chip)}}
ensureLaneChips();
...
$('uiFortify').onclick=e=>{e.stopPropagation();proxyClick('fortifyTower')};
...
// Waypoint targets the lane cards directly. A second Waypoint tap cancels.
...
laneStrip.addEventListener('click',e=>{
 const card=e.target.closest('[data-lane]');if(!card)return;const lane=card.dataset.lane;
 if(targetMode){e.preventDefault();e.stopImmediatePropagation();targetPanel?.querySelector(`[data-target="${lane}"]`)?.click();syncTargetMode();closeTray();return}
 if(e.target.closest('.lane-move-chip')){document.querySelector(`[data-walk="${lane}"]`)?.click();closeTray();return}
 toggleTray(lane);
});

// Outside taps dismiss transient surfaces; no X hunting required.
document.addEventListener('pointerdown',e=>{
 const t=e.target;if(!battle||battle.hidden)return;
 if(openLane&&!tray.contains(t)&&!t.closest('#laneStrip'))closeTray();
 if(readPanel&&!readPanel.hidden&&!readPanel.contains(t)&&t!==readToggle&&!t.closest('#readToggle'))readToggle?.click();
