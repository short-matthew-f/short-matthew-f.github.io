(() => {
  ...
  const FORMAL_ACTIVITY_MIN = Object.freeze({pans:3,pinches:3,commanderOrders:3,recenters:1,north:1,mid:1,south:1});
  ...
  function newEvidence() {
    ...
    return {
      ...
      declarationSnapshot: mode === 'formal-candidate' ? {
        ...
        pointerCancelPolicy:'lifecycle-adjacent | orphaned | interaction-cancel',
        formalActivityMin:{...FORMAL_ACTIVITY_MIN}
      } : null,
      runSetup:{
        mode,
  ...
  if(!evidence.runBaselineCoreCounters) evidence.runBaselineCoreCounters=snapshotCoreCounters();
  ...
  function snapshotCoreCounters() {
    const c=coreEvidence()?.counters||{};
    const out={};
    for(const key of ['commanderOrders','pans','pinches','recenters','contextLosses']) out[key]=Number(c[key]||0);
    return out;
  }
  ...
  function syncCoreCounters() {
    ...
    evidence.runBaselineCoreCounters ||= snapshotCoreCounters();
    for(const key of ['commanderOrders','pans','pinches','recenters','contextLosses']) {
      const relative=Math.max(0,Number(core.counters[key]||0)-Number(evidence.runBaselineCoreCounters[key]||0));
      evidence.counters[key]=Math.max(evidence.counters[key]||0,relative);
    }
  }
  ...
  function formalActivity(attempt=currentStressAttempt()) {
    if(!attempt?.baselineCounters) return {
      pans:0,pinches:0,commanderOrders:0,recenters:0,
      lanes:{north:0,mid:0,south:0},complete:false,missing:['activity baseline']
    };
    syncCoreCounters();
    const b=attempt.baselineCounters;
    const nowLanes=evidence.counters.laneJumpsByLane||{};
    const baseLanes=b.laneJumpsByLane||{};
    const a={
      pans:Math.max(0,(evidence.counters.pans||0)-(b.pans||0)),
      pinches:Math.max(0,(evidence.counters.pinches||0)-(b.pinches||0)),
      commanderOrders:Math.max(0,(evidence.counters.commanderOrders||0)-(b.commanderOrders||0)),
      recenters:Math.max(0,(evidence.counters.recenters||0)-(b.recenters||0)),
      lanes:{
        north:Math.max(0,(nowLanes.north||0)-(baseLanes.north||0)),
        mid:Math.max(0,(nowLanes.mid||0)-(baseLanes.mid||0)),
        south:Math.max(0,(nowLanes.south||0)-(baseLanes.south||0))
      }
    };
    const missing=[];
    if(a.pans<FORMAL_ACTIVITY_MIN.pans) missing.push(`pan ${a.pans}/${FORMAL_ACTIVITY_MIN.pans}`);
    if(a.pinches<FORMAL_ACTIVITY_MIN.pinches) missing.push(`pinch ${a.pinches}/${FORMAL_ACTIVITY_MIN.pinches}`);
    if(a.commanderOrders<FORMAL_ACTIVITY_MIN.commanderOrders) missing.push(`Commander ${a.commanderOrders}/${FORMAL_ACTIVITY_MIN.commanderOrders}`);
    if(a.recenters<FORMAL_ACTIVITY_MIN.recenters) missing.push(`recenter ${a.recenters}/${FORMAL_ACTIVITY_MIN.recenters}`);
    for(const lane of ['north','mid','south']) if(a.lanes[lane]<FORMAL_ACTIVITY_MIN[lane]) missing.push(`${lane} ${a.lanes[lane]}/${FORMAL_ACTIVITY_MIN[lane]}`);
    a.complete=missing.length===0;a.missing=missing;
    attempt.activity=a;
    return a;
  }

  function setupInteractionCancelCount() {
    return (evidence.input?.pointerCancelEvents||[]).filter(x=>x.classification==='interaction-cancel' && x.formalStressAttempt==null).length;
  }
  ...
  function updateStressStatus() {
    ...
    if(evidence.formalStress?.active && attempt) {
      ...
      const activity=formalActivity(attempt);
      if(remain<=0 && !activity.complete) el.textContent=`20m reached · finish timed actions: ${activity.missing.join(' · ')}`;
      else el.textContent=`Attempt ${attempt.attemptNumber} active · ${m}:${String(s).padStart(2,'0')} remaining · ${activity.complete?'timed actions ✓':activity.missing.join(' · ')}`;
  ...
  function beginFormalStress() {
    ...
    if(setupInteractionCancelCount()>0){if(error)error.textContent='Setup captured an active interaction pointer-cancel. Start a fresh declared run before beginning formal Stress.';return;}
  ...
  function completeFormalStress() {
    ...
    attempt.activity=formalActivity(attempt);
  ...
  function sampleFormalStress(raw) {
    ...
    if(attempt.qualifiedForegroundMs>=FORMAL_STRESS_MS) {
      attempt.timeRequirementMet=true;
      const activity=formalActivity(attempt);
      if(activity.complete) completeFormalStress();
    }
  }
  ...
  function frame(now) {
    ...
    window.__LW_EVIDENCE_V4__=evidence;
  ...
  canvas.addEventListener('pointercancel',e=>{
    ...
    const trackedPointer=gesturePointers.has(e.pointerId);
    const activePointerCount=gesturePointers.size;
    const activeGesture=gestureState?.type||null;
    let classification;
    if(document.visibilityState!=='visible'||since<=LIFECYCLE_ADJACENCY_MS) classification='lifecycle-adjacent';
    else if(!trackedPointer && activePointerCount===0 && activeGesture===null) classification='orphaned';
    else classification='interaction-cancel';
    evidence.counters.pointerCancels++;
    const formalStressAttempt=evidence.formalStress?.active?evidence.formalStress.currentAttemptNumber:null;
    evidence.input.pointerCancelEvents.push({
      tMs:t,pointerId:e.pointerId,pointerType:e.pointerType||null,classification,trackedPointer,
      activeGesture,activePointerCount,
    ...
    event('pointercancel',{classification,trackedPointer,activeGesture,activePointerCount,lastLifecycleType,lastLifecycleAgeMs:Number.isFinite(since)?Math.round(since):null,formalStressAttempt});
    gestureEnd(e,'cancel');persist('pointercancel');updateCoverageUI();
    if(classification==='interaction-cancel' && evidence.formalStress?.active) interruptFormalStress('active-interaction-pointer-cancel',{pointerId:e.pointerId});
  },true);
