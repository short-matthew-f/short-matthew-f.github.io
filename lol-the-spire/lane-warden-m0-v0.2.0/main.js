(() => {
  'use strict';
  const BUILD = 'M0-0.2.0';
  const FIXTURE = 'R-01';
  const PARAM_REV = 'R01-A';
  ...
  if (!gl || !R) { fatal.hidden = false; return; }
  ...
  // ---- Parameter set A ------------------------------------------------------
  const PSET = {
    playerPulse: 13.5, enemyPulseNorth: 14.0, enemyPulseSouth: 12.5,
    guardHp: 900, guardRegen: 14, gateHp: 2300, bastionHp: 1100, coreHp: 1450,
    presenceRadius: 13, presenceDamage: 1.28, fullGold:8, remoteGold:3,
    waypointCooldown:45, rallyCooldown:22, rallySeconds:7, rallyDamage:1.32,
    commanderHp:420, commanderDamage:19, commanderRange:7.5, commanderCadence:.78, reformSeconds:9,
    junctionX:-18, initialGold:60
  };
  const ENEMY = {
    raider:{hp:100,damage:11,range:2,speed:3.6,cadence:.85,role:'melee',structure:1},
    archer:{hp:65,damage:10,range:7.5,speed:3.15,cadence:1.0,role:'ranged',structure:.65},
    brute:{hp:170,damage:14,range:2.2,speed:2.55,cadence:.8,role:'melee',structure:1.45}
  };
  ...
  const world={time:0,actors:[],projectiles:[],lanes:{north:newLane('north'),south:newLane('south')},gate:{hp:PSET.gateHp,max:PSET.gateHp,vulnerable:false},core:{hp:PSET.coreHp,max:PSET.coreHp},commander:{x:-46,y:11,lane:'north',hp:PSET.commanderHp,max:PSET.commanderHp,path:[],attackCd:0,incapacitated:false,reformAt:0},gold:PSET.initialGold,nextPlayerPulse:1.0,nextEnemy:{north:3.5,south:5.5},rallyReady:0,rallyUntil:0,waypointReady:0,result:null};

  const evidence={schema:1,build:BUILD,designBaseline:'1.7',fixture:FIXTURE,parameterRevision:PARAM_REV,parameterSet:PSET,startedAt:new Date().toISOString(),runs:[],events:[]};
  let currentRun=null;
  ...
  function spawnEnemyPulse(laneId){
    const recipe=laneId==='north'?['raider','raider','archer']:['raider','brute','raider','archer'];let offset=0;for(const id of recipe){spawnActor(-1,laneId,id,58+offset);offset+=1.5;}currentRun.metrics.enemyPulses[laneId]++;ev('enemy-pulse',{lane:laneId,count:recipe.length});
  }
  ...
  function damageStructure(obj,dmg,laneId,kind){if(obj.broken||obj.hp<=0)return;obj.hp-=dmg;if(kind==='guard')obj.hits.push({t:world.time,d:dmg});if(kind==='bastion')obj.hits.push({t:world.time,d:dmg});if(obj.hp<=0){obj.hp=0;obj.broken=true;if(kind==='guard'){if(!world.gate.vulnerable){world.gate.vulnerable=true;currentRun.metrics.guardBreakLane=laneId;ev('gate-vulnerable',{lane:laneId});}ev('guard-broken',{lane:laneId});}if(kind==='bastion'){currentRun.metrics.bastionBreaks.push({lane:laneId,t:+world.time.toFixed(1)});ev('bastion-broken',{lane:laneId});}}}
  ...
  function finishBattle(result){if(world.result)return;world.result=result;phase='ended';currentRun.result={result,duration:+world.time.toFixed(1),gateHp:+world.gate.hp.toFixed(1),coreHp:+world.core.hp.toFixed(1),gold:world.gold,guardBreakLane:currentRun.metrics.guardBreakLane,bastionBreaks:currentRun.metrics.bastionBreaks};currentRun.endedAt=new Date().toISOString();ev('battle-end',currentRun.result);$('battleResult').hidden=false;$('resultTitle').textContent=result==='win'?'Gate broken':'Core fallen';$('resultBody').textContent=result==='win'?'The breach converted. Now ask whether you won because you concentrated—or because the other lane never became a meaningful clock.':'The fork closed against you. Inspect which lane was making real Guard progress and which clock you misread.';$('resultStats').innerHTML=`<b>${Math.floor(world.time/60)}:${String(Math.floor(world.time%60)).padStart(2,'0')} battle</b><b>${currentRun.metrics.guardBreakLane||'no'} breach</b><b>${Math.round(world.core.hp/world.core.max*100)}% Core</b>`;updateUI();}

  // ---- Abilities / economy --------------------------------------------------
  $('rallyButton').onclick=()=>{if(phase!=='battle'||world.commander.incapacitated||world.time<world.rallyReady)return;world.rallyUntil=world.time+PSET.rallySeconds;world.rallyReady=world.time+PSET.rallyCooldown;ev('rally',{lane:world.commander.lane});};
  $('waypointButton').onclick=()=>{if(phase!=='battle'||world.commander.incapacitated||world.time<world.waypointReady)return;waypointTargeting=!waypointTargeting;$('waypointButton').classList.toggle('active',waypointTargeting);updateUI();};
  $('pushButton').onclick=()=>{if(phase!=='battle'||world.gold<60)return;const types=R.committedTypes(deployment,selectedLane);if(!types.length)return;world.gold-=60;currentRun.metrics.goldSpent+=60;const n=spawnPlayerPulse(selectedLane,'gold-push');ev('gold-spend',{kind:'push',lane:selectedLane,cost:60,count:n});updateUI();};
  $('overdriveButton').onclick=()=>{if(phase!=='battle'||world.gold<60)return;const lane=world.lanes[selectedLane];if(!lane.towers.some(t=>!t.dead))return;world.gold-=60;currentRun.metrics.goldSpent+=60;lane.overdriveUntil=world.time+12;ev('gold-spend',{kind:'tower-overdrive',lane:selectedLane,cost:60,seconds:12});updateUI();};
  ...
  canvas.addEventListener('pointerdown',e=>{if(phase!=='battle')return;canvas.setPointerCapture?.(e.pointerId);pointers.set(e.pointerId,{sx:e.clientX,sy:e.clientY,x:e.clientX,y:e.clientY});if(pointers.size===1)gesture={type:'pending'};else if(pointers.size===2){const p=[...pointers.values()];gesture={type:'pinch',dist:Math.hypot(p[0].x-p[1].x,p[0].y-p[1].y),zoom:camera.zoom};}});
  ...
  $('audioButton').onclick=()=>{try{const A=window.AudioContext||window.webkitAudioContext;const ac=new A();const o=ac.createOscillator(),g=ac.createGain();o.frequency.value=420;g.gain.value=.035;o.connect(g);g.connect(ac.destination);o.start();g.gain.exponentialRampToValueAtTime(.001,ac.currentTime+.12);o.stop(ac.currentTime+.13);$('audioButton').textContent='Sound ready';if(currentRun)ev('audio-unlock');}catch(_){$('audioButton').textContent='Sound error';}};
  $('exportEvidence').onclick=()=>{const snapshot={...evidence,exportedAt:new Date().toISOString(),current:{phase,time:+world.time.toFixed(2),gold:world.gold,gateHp:world.gate.hp,coreHp:world.core.hp,actors:world.actors.length,selectedLane}};const blob=new Blob([JSON.stringify(snapshot,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`lane-warden-${BUILD}-${PARAM_REV}-gameplay-${new Date().toISOString().replace(/[:.]/g,'-')}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);};
