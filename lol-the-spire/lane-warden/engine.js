...
function createBattle(run,deployment=defaultDeployment()){
 ...
 return{schema:1,seed:run.seed,nodeId:'A1-B1',elapsed:0,gold:42,core:100,gate:100,gateVulnerable:false,gateOpenedAt:null,commanderLane:'north',commanderDown:0,readReserve:4,readMenuOpen:false,lastStand:null,result:null,
...
function stepBattle(b,dt){
 ...
 if(b.readMenuOpen&&b.readReserve>0)b.readReserve=Math.max(0,b.readReserve-dt);else if(!b.readMenuOpen)b.readReserve=Math.min(4,b.readReserve+dt*.22);
 ...
 for(const[id,l]of Object.entries(b.lanes)){
  ...
  const rally=b.effects[id].rally>0?1.35:1,cons=b.effects[id].conscript>0?1.3:1;
  b.effects[id].rally=Math.max(0,b.effects[id].rally-sd);b.effects[id].conscript=Math.max(0,b.effects[id].conscript-sd);
  const friendly=l.allyPower*presence*rally*cons,pressure=friendly-l.enemyPower;
  l.front=Math.max(.08,Math.min(.92,l.front+pressure*sd*.0029));
  if(pressure>0){
   if(l.bastion>0)l.bastion=Math.max(0,l.bastion-pressure*sd*1.08);
   else if(l.guard>0){const dmg=pressure*sd*.88;l.guard=Math.max(0,l.guard-dmg);if(l.guard>0)l.guard=Math.min(100,l.guard+l.regen*sd*.42)}
   else gateDamage+=Math.max(0,friendly-l.enemyPower*.35)*sd*.72;
  }else{
   const threat=(-pressure)*sd*.22;if(l.front<.25)b.core=Math.max(0,b.core-threat*1.6);l.front=Math.max(.08,l.front-threat*.003);
  }
  l.status=bastionState(l.bastion);
 }
 const vulnerable=Object.values(b.lanes).some(l=>l.guard<=0);
 if(vulnerable&&!b.gateVulnerable){b.gateOpenedAt=b.elapsed;b.events.push({t:b.elapsed,type:'gate-vulnerable'})}
 b.gateVulnerable=vulnerable;
 if(b.gateVulnerable&&gateDamage>0)b.gate=Math.max(0,b.gate-gateDamage);
...
function useAbility(b,id,targetLane){if(b.result||b.lastStand?.active)return false;if((b.abilities[id]||0)>0)return false;const lane=targetLane||b.commanderLane;if(!b.lanes[lane])return false;const def=D().commander.abilities.find(a=>a.id===id);if(!def)return false;
 if(id==='rally')b.effects[lane].rally=6;
 if(id==='sunder'){const l=b.lanes[lane];if(l.bastion>0)l.bastion=Math.max(0,l.bastion-12);else if(l.guard>0)l.guard=Math.max(0,l.guard-18);else if(b.gateVulnerable)b.gate=Math.max(0,b.gate-16)}
 if(id==='waypoint')b.commanderLane=lane;if(id==='conscript')b.effects[lane].conscript=8;
...
function selfTest(){
 const r=newRun('regression-seed'),b=createBattle(r);
 const gateNoPassive=(()=>{const x=createBattle(newRun('gate-passive'));x.lanes.north.bastion=0;x.lanes.north.guard=0;x.lanes.north.allyPower=0;x.lanes.north.enemyPower=5;const before=x.gate;stepBattle(x,1);return x.gate===before&&x.gateVulnerable})();
 const sunderHitsGate=(()=>{const x=createBattle(newRun('gate-sunder'));x.lanes.north.bastion=0;x.lanes.north.guard=0;x.gateVulnerable=true;const before=x.gate;useAbility(x,'sunder','north');return x.gate<before})();
 const checks={twoLanes:Object.keys(b.lanes).length===2,zeroEmberLegal:(()=>{const rr=newRun('z');rr.embers=0;return rr.embers===0})(),guardRegen:true,gateNeedsGuard:!b.gateVulnerable,gateNoPassive,sunderHitsGate,deterministic:(()=>{const a=createBattle(newRun('x')),c=createBattle(newRun('x'));return a.lanes.north.enemyPower===c.lanes.north.enemyPower})()};return{pass:Object.values(checks).every(Boolean),checks}}
